...
public class Scenario {
    ...
    public Scenario(int nodes, double transmissionRange, double topologySize, int slots, int connections, SchedulerType type) {
        this.nodes = nodes;
        ...
        for (int i = 0; i < connections; i++) {
            ...
            trafficGenerators.add(new TrafficGenerator(src, dst, simulator, 100, 5 * scheduller.slotTime, 1.0));
        }
        
        
        //network.nodes.get(0).receive(new Packet(network.nodes.get(0), network.nodes.get(network.nodes.size() - 1)));
//        trafficGenerators.add(new TrafficGenerator(network.nodes.get(0), network.nodes.get(network.nodes.size() - 1), simulator, 100, 5 * scheduller.slotTime, 1.0));
//        trafficGenerators.add(new TrafficGenerator(network.nodes.get(1), network.nodes.get(network.nodes.size() - 2), simulator, 100, 5 * scheduller.slotTime, 1.0));
//        trafficGenerators.add(new TrafficGenerator(network.nodes.get(2), network.nodes.get(network.nodes.size() - 3), simulator, 100, 5 * scheduller.slotTime, 1.0));
//        trafficGenerators.add(new TrafficGenerator(network.nodes.get(3), network.nodes.get(network.nodes.size() - 4), simulator, 100, 5 * scheduller.slotTime, 1.0));
//        trafficGenerators.add(new TrafficGenerator(network.nodes.get(4), network.nodes.get(network.nodes.size() - 5), simulator, 100, 5 * scheduller.slotTime, 1.0));
        simulator.run();
